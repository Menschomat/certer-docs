...
Run the built-in [`keygen`](../../cli/keygen/) utility inside the Certer container to generate a secure hash:
...
docker run -d \
  ...
  -e CF_DNS_API_TOKEN="your_cloudflare_dns_api_token" \
  ghcr.io/menschomat/certer:latest
```

### Verified Run Parameters:
*   `-p 8080:8080`: Exposes the REST API on port `8080`.
*   `-v ...:/certs`: Mounts a folder where ACME certificates and private key files will be written.
*   `-v ...:/config.json`: Mounts the configuration schema.
*   `-e DNS_PROVIDER=...` and provider token variables: Configure the DNS-01 solver.
